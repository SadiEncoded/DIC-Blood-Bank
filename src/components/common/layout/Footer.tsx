...
import { CONTACT_INFO, DEVELOPER_INFO } from '@/assets/data/contact';
...
export default function Footer() {
  ...
  const resources = [
    ...
    { label: 'FAQs', href: '/faq' },
    { label: 'Emergency Request', href: '/donors' },
  ];
  ...
  return (
    <footer className="bg-slate-50 border-t border-slate-200">
      <div className="max-w-7xl mx-auto px-6 py-10 md:py-14">
        ...
        {/* Main Grid: 2 columns on mobile, 4 columns on large screens */}
        <div className="grid grid-cols-2 lg:grid-cols-12 gap-x-6 gap-y-10 md:gap-12 mb-10 md:mb-16">

          {/* Brand - Full width on mobile, 4/12 on desktop */}
          <div className="col-span-2 lg:col-span-4 space-y-5 md:space-y-6">
            <div className="relative w-40 md:w-48 h-10 md:h-12"> {/* Adjusted height to prevent logo squashing */}
              <Image
            ...
            <div className="space-y-2.5 md:space-y-3 text-xs md:text-sm text-slate-600">
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-rose-50 flex items-center justify-center shrink-0">
                   <CONTACT_INFO.address.icon size={12} className="text-rose-600 md:w-[14px] md:h-[14px]" />
                </div>
                <span className="truncate">{CONTACT_INFO.address.full}</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-rose-50 flex items-center justify-center shrink-0">
                  <Phone size={12} className="text-rose-600 md:w-[14px] md:h-[14px]" />
                </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {CONTACT_INFO.socials.map((link: any, i: number) => {
          ...
          <div className="col-span-1 lg:col-span-2">
            <h4 className="text-[9px] md:text-[10px] font-bold text-slate-900 uppercase tracking-[0.2em] mb-4 md:mb-6">
              Quick Links
            </h4>
            ...
            <div className="bg-white border border-slate-200 rounded-xl md:rounded-2xl p-4 md:p-5 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] md:text-sm font-bold text-slate-900 truncate">
                    {DEVELOPER_INFO.name}
                  </p>
                  <p className="text-[9px] md:text-[10px] text-slate-500 flex items-center gap-1 uppercase tracking-tight">
                    <Code2 size={10} className="text-rose-500 md:w-3 md:h-3" />
                    {DEVELOPER_INFO.role}
                  </p>
                </div>
